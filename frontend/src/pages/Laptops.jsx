...
const empty = () => ({
  numero_serie: "", marca: "", modelo: "", procesador: "",
...
export default function Laptops() {
  ...
  useEffect(() => {
    Promise.all([
      api.getMarcas(), api.getModelos(), api.getProcesadores(),
      api.getRam(), api.getAlmacenamientos(), api.getEmpleados(),
    ]).then(([m, mo, p, r, a, e]) => {
      setMarcas(m); setModelos(mo); setProcesadores(p);
      setRams(r); setAlmacenamientos(a); setEmpleados(e.filter((x) => x.activo));
    });
  }, []);
  ...
  const openEdit = (lp) => {
    setEditId(lp.id);
    setForm({
      numero_serie: lp.numero_serie,
      marca: lp.marca.id,
      modelo: lp.modelo.id,
      procesador: lp.procesador.id,
      ram: lp.ram.id,
      almacenamiento: lp.almacenamiento.id,
      estado: lp.estado,
      empleado_asignado: lp.empleado_asignado?.id || "",
      notas: lp.notas || "",
    });
    setErrors({});
    setModal(true);
  ...
  const submit = async (e) => {
    ...
    try {
      editId ? await api.updateLaptop(editId, payload) : await api.createLaptop(payload);
      setModal(false);
      fetch_();
  ...
  const confirmDelete = async () => {
    await api.deleteLaptop(delId).catch(console.error);
    setDelId(null);
    fetch_();
  };
  ...
  const openHist = async (id) => {
    setHistModal(id);
    const d = await api.laptopHistorial(id).catch(() => []);
    setHistData(d);
  };
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div className="flex flex-wrap gap-2">
          <button onClick={() => api.laptopsExcel().then((b) => downloadBlob(b, "laptops.xlsx"))}
          ...
          <button onClick={() => api.laptopsPdf().then((b) => downloadBlob(b, "laptops.pdf"))}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b">
              <tr>
                {["N° Serie", "Marca / Modelo", "Procesador", "RAM", "Disco", "Estado", "Asignado a", "Acciones"].map((h, i) => (
                  <th key={h} className={`text-left px-4 py-3 font-semibold text-gray-600 ${i >= 2 && i <= 4 ? "hidden lg:table-cell" : ""} ${i === 6 ? "hidden md:table-cell" : ""} ${i === 7 ? "text-right" : ""}`}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y">
              {loading ? (
                ...
                <tr key={lp.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3 font-mono text-xs">{lp.numero_serie}</td>
                  <td className="px-4 py-3">
                    <div className="font-medium">{lp.marca.nombre}</div>
                    <div className="text-gray-500 text-xs">{lp.modelo.nombre}</div>
                  </td>
                  <td className="px-4 py-3 hidden lg:table-cell text-gray-600">{lp.procesador.nombre}</td>
                  <td className="px-4 py-3 hidden lg:table-cell text-gray-600">{lp.ram.capacidad}</td>
                  <td className="px-4 py-3 hidden lg:table-cell text-gray-600">{lp.almacenamiento.capacidad}</td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${BADGE[lp.estado]}`}>{lp.estado_display}</span>
                  </td>
                  <td className="px-4 py-3 hidden md:table-cell">
                    {lp.empleado_asignado
                      ? <><div className="text-xs font-medium">{lp.empleado_asignado.nombre}</div><div className="text-xs text-gray-400">{lp.empleado_asignado.area_nombre}</div></>
                      : <span className="text-xs text-gray-400">Sin asignar</span>}
                  </td>
                  <td className="px-4 py-3 text-right">
                    <div className="flex justify-end gap-1">
                      <button onClick={() => openHist(lp.id)} className="p-1.5 text-gray-400 hover:text-blue-600 rounded"><History className="w-4 h-4" /></button>
                      <button onClick={() => openEdit(lp)} className="p-1.5 text-gray-400 hover:text-amber-600 rounded"><Edit className="w-4 h-4" /></button>
                      <button onClick={() => setDelId(lp.id)} className="p-1.5 text-gray-400 hover:text-red-600 rounded"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {/* Modal Crear/Editar */}
      <Modal open={modal} onClose={() => setModal(false)} title={editId ? "Editar Laptop" : "Nueva Laptop"} wide>
        <form onSubmit={submit} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">N° Serie *</label>
              <input type="text" required value={form.numero_serie}
            ...
            <Sel label="Procesador" name="procesador" options={procesadores} display={(o) => o.nombre} required />
      ...
      <Modal open={!!delId} onClose={() => setDelId(null)} title="Confirmar Eliminación">
        <p className="text-gray-600 mb-6">¿Estás seguro de eliminar esta laptop? No se puede deshacer.</p>
